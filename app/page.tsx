...
type LastSelectedAppsType = AppType[];

type SelectedAppType = AppType | null;
...
function App() {
  ...
  const [selectedApp, setSelectedApp] = useState<SelectedAppType>(null);
  const [lastSelectedApps, setLastSelectedApps] = useState<LastSelectedAppsType>([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      const appsByAppId = apps.reduce<Record<string, AppType>>((acc, app) => {
        acc[app.app_id] = app;
        return acc;
      }, {});
  ...
  function handleSelectedApp(app: AppType) {
    ...
    // Remove o app se já estiver na lista
    const filteredLastSelectedApps = lastSelectedApps.filter((a) => a.app_id !== app.app_id);
    const newLastSelectedApps = [...filteredLastSelectedApps, app].slice(-3);
            ...
            <div className='grid grid-cols-4 gap-6'>
              {pagedFilteredApps.map((app) => (
                <a
                  ...
                  className='group card cursor-pointer bg-base-100 shadow-sm transition card-sm hover:shadow-lg'
                >
